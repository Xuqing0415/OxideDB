<!-- 发布信息：发布时复制到 CSDN 的表单里，正文从下面的一级标题开始。

标题：收集了不检查，等于没做：一个躲过 96 个测试的 Raft 读 bug
备选标题（SEO 更全）：Raft ReadIndex 踩坑记：一个躲过 96 个测试的线性一致性读 bug

摘要（约 100 字，填 CSDN 的「摘要」框）：
一个 Raft 读路径收集了所有 AppendEntries 响应，却从不检查它们，96 个测试全绿。
直到网络分区之后，它返回了一个自己没有资格给出的答案。本文从这段真实代码讲到
ReadIndex 的多数派论证，以及新领导者为什么必须先提交一条自己任期的日志。

标签（5 个）：Raft、分布式系统、线性一致性、Python、数据库
分类专栏建议：分布式系统 / 数据库
封面建议：无图也可发布；若要封面，用深色底 + 一行等宽字：response.success && response.term == current_term
...
t4   旧实现：A 返回本地的 v      ← 陈旧读，而且客户端无法察觉
     正确实现：A 拿不到多数派确认，拒绝这次读
t5   A 恢复连接，收到更高任期，降级为 follower，日志向新领导者对齐
     （未提交的冲突后缀被截断，本地 commit_index 与 last_applied 随之收回）
     —— 但 t3 已经发给客户端的那个 v，收不回来了
```
...
仓库是 [github.com/Xuqing0415/OxideDB](https://github.com/Xuqing0415/OxideDB)（Python，存储层只用标准库，`pip install -e ".[test]"`）。最小复现就是本文开头那段输出：
